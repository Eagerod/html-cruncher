...
HTMLElement.findTagInString = function(buffer) {
    ...
    do {
        if (buffer[i] === "<") {
            ...
            // Deal with cases where closing > may appear inside strings.
            var term = buffer[i];
            do {
                ++i;
            } while (buffer[i] !== term && i < buffer.length);
        }
        ++i;
...
HTMLElement.processAttributesFromString = function(attributeString, elementAttributes) {
    while ( attributeString.length ) {
        ...
        if (attributeString.charAt(0) === "=") {
            ...
            var attribute = new HTMLElement(HTMLElement.DataTypes.Attribute, attributeValue[0].replace(/("|')/g, ""));
            elementAttributes[attributeName[0]] = attribute;
            attributeString = attributeString.substring(attributeValue[0].length).trim();
        }
        else {
            elementAttributes[attributeName] = new HTMLElement(HTMLElement.DataTypes.Attribute);
        }
    }
};
