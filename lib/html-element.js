...
HTMLElement.prototype._toString = function(indent) {
    ...
    else if ( this.dataType === HTMLElement.DataTypes.Attribute ) {
        // Case where an attribute is asked for its own string value, rather than building through a tag.
        toString += this.content;
    }
    else { // Root element
...
HTMLElement._xpath = function (filteredElements, remainingQuery) {
    // Before doing anything with this list of filteredElements, uniqueify it so
    // we don't try to search anything more than once.
    var elements = [];
    for (var i in filteredElements) {
...
HTMLElement._xpathPredicate = function(filteredElements, predicate) {
    ...
    var match = predicate.match(/^position\(\)\s*=\s*(size\(\)(?:\s*-\s*\d+)?|\d+)$/);
    if (match) {
        var offset = parseInt(match[1]);
        if (isNaN(offset)) {
            // size() in the query
            offset = filteredElements.length + parseInt(match[1].replace("size()", "").replace(/\s/g, ""));
        }
        if (isNaN(offset)) {
            // size() is the query.
            return [filteredElements[filteredElements.length - 1]];
        }
        return [filteredElements[parseInt(offset) - 1]];
    }
